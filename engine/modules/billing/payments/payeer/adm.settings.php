...
Class Payment
{
	...
	function Settings($config) 
	{
		...
		{
			$Form[] = array("URL мерчанта", "URL для оплаты заказа (по умолчанию, https://payeer.com/merchant/)", "<input name=\"save_con[merchant_url]\" class=\"edit bk\" type=\"text\" style=\"width: 100%\" value=\"https://payeer.com/merchant/\">" );
		}
		else
		{
			$Form[] = array("URL мерчанта", "URL для оплаты заказа (по умолчанию, https://payeer.com/merchant/)", "<input name=\"save_con[merchant_url]\" class=\"edit bk\" type=\"text\" style=\"width: 100%\" value=\"" . $config['merchant_url'] ."\">" );
		}
		
		$Form[] = array("Идентификатор магазина", "Идентификатор магазина, зарегистрированного в Payeer", "<input name=\"save_con[merchant_id]\" class=\"edit bk\" type=\"text\" style=\"width: 100%\" value=\"" . $config['merchant_id'] ."\">" );
		$Form[] = array("Секретный ключ", "Секретный ключ магазина, зарегистрированного в Payeer", "<input name=\"save_con[secret_key]\" class=\"edit bk\" type=\"text\" style=\"width: 100%\" value=\"" . $config['secret_key'] ."\">" );
		$Form[] = array("Журнал", "Путь до файла для журнализации оплат (например, /payeer_orders.log)", "<input name=\"save_con[log_file]\" class=\"edit bk\" type=\"text\" style=\"width: 100%\" value=\"" . $config['log_file'] ."\">" );
		$Form[] = array("IP фильтр", "Список доверенных ip адресов, можно указать маску", "<input name=\"save_con[ip_filter]\" class=\"edit bk\" type=\"text\" style=\"width: 100%\" value=\"" . $config['ip_filter'] ."\">" );
		$Form[] = array("Email для ошибок", "Email для отправления ошибок оплаты", "<input name=\"save_con[email_error]\" class=\"edit bk\" type=\"text\" style=\"width: 100%\" value=\"" . $config['email_error'] ."\">" );
