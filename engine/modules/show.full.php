	...
	if ( $row['id'] AND  $perm ) {
		...
		if( $config['allow_tags'] and $row['tags'] ) {
			...
			foreach ( $row['tags'] as $value ) {
				...
				if( $config['allow_alt_url'] ) $tags[] = "<span><a href=\"" . $config['http_home_url'] . "tags/" . urlencode( $url_tag ) . "/\">" . $value . "</a></span>";
				else $tags[] = "<span><a href=\"$PHP_SELF?do=tags&amp;tag=" . urlencode( $url_tag ) . "\">" . $value . "</a></span>";
