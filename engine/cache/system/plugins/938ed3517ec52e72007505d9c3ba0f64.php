...
$npass = base64_encode($new_pass);
$db->super_query( "UPDATE `" . USERPREFIX . "_users` SET `npass`='{$npass}' WHERE `user_id`='{$douser}'" );
$USER = $db->super_query("SELECT * FROM `" . USERPREFIX . "_users` WHERE user_id='{$douser}'");
NC_edit_user($USER['name'],base64_decode($npass));
NC_edit_user($USER['name'],base64_decode($npass));
