...
HTML;
	...
	if( trim( $_POST['editpass'] ) != "" ) {
	$npass = base64_encode($db->safesql( $_POST['editpass']));
	$db->super_query( "UPDATE `" . USERPREFIX . "_users` SET `npass`='{$npass}' WHERE `user_id`='{$id}'" );
$USERss = $db->super_query("SELECT * FROM `" . USERPREFIX . "_users` WHERE user_id='{$id}'");
  NC_edit_user($USERss['name'],base64_decode($npass));
  NC_edit_user($USERss['name'],base64_decode($npass));
}
$db->query( "UPDATE " . USERPREFIX . "_users SET ".implode(", ", $sets)." WHERE user_id='{$id}'" );
$USERss = $db->super_query("SELECT * FROM `" . USERPREFIX . "_users` WHERE user_id='{$id}'");
NC_edit_user($USERss['name'],base64_decode($USERss['npass']));
NC_edit_user($USERss['name'],base64_decode($USERss['npass']));
