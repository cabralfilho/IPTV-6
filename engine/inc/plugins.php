...
if( $_GET['action'] == "delete" ) {
	...
	$row = $db->super_query( "SELECT id, mysqldelete, filedelete, filelist FROM " . PREFIX . "_plugins WHERE id='{$id}'" );
...
if( $_REQUEST['action'] == "download" ) {
	...
	while ( $filerow = $db->get_row() ) {
		...
		$files[$filerow['file']][] = array('action' => $filerow['action'], 'searchcode' => $filerow['searchcode'], 'replacecode' => $filerow['replacecode'], 'searchcount' => intval($filerow['searchcount']) );
	}
...
if( $_REQUEST['action'] == "doadd" OR $_REQUEST['action'] == "doedit" ) {
	...
	if( $_REQUEST['action'] == "doadd" ) {
		...
		$db->query( "INSERT INTO " . PREFIX . "_plugins (name, description, icon, version, dleversion, versioncompare, active, mysqlinstall, mysqlupgrade, mysqlenable, mysqldisable, mysqldelete, filedelete, upgradeurl) values ('{$name}', '{$description}','{$icon}','{$version}','{$dleversion}','{$versioncompare}', '1', '{$mysqlinstall}', '{$mysqlupgrade}','{$mysqlenable}','{$mysqldisable}','{$mysqldelete}', '{$filedelete}', '{$upgradeurl}')" );
		$id = $db->insert_id();
...
if( $_REQUEST['action'] == "add" OR $_REQUEST['action'] == "edit" ) {
	...
	if( $_REQUEST['action'] == "add" ) {
		...
		while ( $filerow = $db->get_row() ) {
			...
			$filerow['file'] = htmlspecialchars( $filerow['file'], ENT_QUOTES, $config['charset'] );
			$files[$filerow['file']][] = array('action' => htmlspecialchars($filerow['action'], ENT_QUOTES, $config['charset'] ), 'searchcode' => htmlspecialchars($filerow['searchcode'], ENT_QUOTES, $config['charset'] ), 'replacecode' => htmlspecialchars($filerow['replacecode'], ENT_QUOTES, $config['charset'] ), 'searchcount' => intval($filerow['searchcount']) );
		}
